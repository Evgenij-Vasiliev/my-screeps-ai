...
o.boosted=bo;
var lw=null;for(var n2 in Game.creeps){var c2=Game.creeps[n2];if(c2.memory.role==='labWorker')lw=lw||{};if(c2.memory.role==='labWorker'&&c2.room.name==='E35S37')lw={task:c2.memory.task||null,res:c2.memory.resource||null,ttl:c2.ticksToLive};}
o.labWorker=lw;
console.log('DBG5'+JSON.stringify(o));})();`;
(async () => {
  ...
  api.socket.subscribe("console", (ev) => {
    ...
    [].concat(m.log||[]).concat(m.error||[]).concat(m.result||[]).forEach((raw) => {
      ...
      if (line.indexOf("DBG5") === 0) console.log(line.slice(4));
    });
  });
  for (let i=0;i<4;i++){ await api.console(CMD, "shard3"); await new Promise(r=>setTimeout(r,60000)); }
