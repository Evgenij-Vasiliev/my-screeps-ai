...
function testLabSource() {
  ...
  // 4. Внутрикомнатный перенос попадает в задачу labWorker: ZHO2 нужен
  //    тройке labs2, но лежит он в лаборатории третьей тройки (для неё ZHO2 —
  ...
  //    ВАЖНО: ресурс источника обязан совпадать с memory.resource задачи —
  //    v6 проверяет это перед withdraw (источник исчерпан -> задача сброшена),
  //    поэтому здесь в lab1r3 лежит именно ZHO2, а не XZHO2.
  nextTick();
  room.storage.store = new Store(1000000, {});
  room.terminal.store = new Store(300000, {});
  lab1r3.store = new Store(3000, { ZHO2: 3000 });
  ...
  const loadCreep = name => {
    ...
    // В рюкзаке энергия, но нужного реагента нет — это фаза забора: крип едет
    // к источнику. Задача здесь не сбрасывается (v6: пустой по нужному
    // ресурсу рюкзак — нормальное состояние фазы забора, см. lab.worker.js).
    ...
    c.withdraw = function (target, resource, amount) {
      ...
      // Как движок: ресурс реально переезжает в рюкзак крипа (следующим
      // вызовом крип уже везёт его и едет к лаборатории-получателю).
